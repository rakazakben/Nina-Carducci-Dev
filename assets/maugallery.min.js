...
element.wrap(`<div class='item-column mb-4${columnClasses}'></div>`)}else{console.error(`Columns should be defined as numbers or objects. ${typeof columns} is not supported.`)}},moveItemInRowWrapper(element){element.appendTo(".gallery-items-row")},responsiveImageItem(element){if(element.prop("tagName")==="IMG"){element.addClass("img-fluid")}},openLightBox(element,lightboxId){$(`#${lightboxId}`).find(".lightboxImage").attr("src",element.attr("src"));$(`#${lightboxId}`).modal("toggle")},prevImage(){let images=$("img.gallery-item:visible");if(images.length===0)return;let activeImage=$(".lightboxImage").attr("src");let index=images.map((i,img)=>$(img).attr("src")).get().indexOf(activeImage);if(index!==-1){let prevIndex=(index-1+images.length)%images.length;$(".lightboxImage").attr("src",$(images[prevIndex]).attr("src"))}},nextImage(){let images=$("img.gallery-item:visible");if(images.length===0)return;let activeImage=$(".lightboxImage").attr("src");let index=images.map((i,img)=>$(img).attr("src")).get().indexOf(activeImage);if(index!==-1){let nextIndex=(index+1)%images.length;$(".lightboxImage").attr("src",$(images[nextIndex]).attr("src"))}},createLightBox(gallery,lightboxId,navigation){gallery.append(`<div class="modal fade" id="${
